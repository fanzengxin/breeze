...
public interface MenuDao extends BaseDao {
    ...
    @Select(sql = "select id, menu_name, menu_type, menu_icon, menu_permission, menu_url, menu_open_way, parent_menu_id, sort_no" +
            " from sys_menu where menu_type < 3 and menu_permission in ({for:permissions:for}) or menu_permission = '' order by sort_no")
    DataList getMenuTree(List<String> permissions, Serial serial);
