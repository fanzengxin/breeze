...
public class UtilSqlFormat {
    ...
    public static Map<String, Object> prepareSql(String sql, Map<String, Object> params) {
        ...
        while (resultSql.indexOf(":$") != -1) {
            ...
            if (params.get(param) != null && params.get(param) instanceof String) {
                resultSql = resultSql.replace(":$" + param, "'" + params.get(param) + "'");
            } else {
                resultSql = resultSql.replace(":$" + param, String.valueOf(params.get(param)));
            }
        }
    ...
    private static String getSql(String sql, Map<String, Object> params) {
        ...
        for (int i = 0; i < sqls.length; i++) {
            ...
            if (subSqls[0].startsWith(":?")) {
                ...
                // 占位符条件判断
                String field = UtilString.subString(subSqls[0], ":#", " ").replaceAll("%", "");
     ...
     * @return
     */
    private static String pareSql(String sql, Object o, int i) {
        if (o != null && !"".equals(o)) {
            return UtilString.subString(sql, "{$" + i + "}", "{/$" + i + "}", false);
        } else {
            return "";
        }
    }
